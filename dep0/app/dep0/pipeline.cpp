...
{
    TRACE_EVENT("parsing", "parser_pipeline_t::run()", "file", f.native());
...
dep0::expected<dep0::typecheck::module_t> typecheck_pipeline_t::run(std::filesystem::path const& f) const
{
    ...
    TRACE_EVENT("typechecking", "typecheck_pipeline_t::run()", "file", f.native());
...
dep0::expected<dep0::typecheck::module_t> transform_pipeline_t::run(std::filesystem::path const& f) const
{
    ...
    {
        TRACE_EVENT("transform", "transform_pipeline_t::run()", "file", f.native());
        auto const result =
            dep0::transform::run(
                *module,
...
dep0::expected<dep0::unique_ref<llvm::Module>> llvmgen_pipeline_t::run(std::filesystem::path const& f) const
{
    ...
    if (not module)
        return module.error();
    TRACE_EVENT("llvmgen", "llvmgen_pipeline_t::run()", "file", f.native());
...
dep0::expected<dep0::temp_file_t> compile_pipeline_t::run(std::filesystem::path const& f) const
{
    ...
    if (not module)
        return module.error();
    TRACE_EVENT("compile", "compile_pipeline_t::run()", "file", f.native());
    auto result =
