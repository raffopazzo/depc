...
boost::test_tools::predicate_result is_typename(dep0::parser::sort_t const& sort)
{
...
boost::test_tools::predicate_result is_type_var(dep0::parser::sort_t const& sort, std::string_view const name)
{
    if (auto const type = std::get_if<dep0::parser::type_t>(&sort))
...
boost::test_tools::predicate_result is_type_i32(dep0::parser::type_t const& type)
{
    if (std::holds_alternative<dep0::parser::type_t::i32_t>(type.value))
        return true;
    else
    {
        auto failed = boost::test_tools::predicate_result(false);
...
boost::test_tools::predicate_result is_type_u32(dep0::parser::type_t const& type)
{
    if (std::holds_alternative<dep0::parser::type_t::u32_t>(type.value))
        return true;
    else
    {
        auto failed = boost::test_tools::predicate_result(false);
...
boost::test_tools::predicate_result is_numeric_constant(dep0::parser::expr_t const& expr, std::string_view const x)
{
    auto failed = boost::test_tools::predicate_result(false);
    auto const c = std::get_if<dep0::parser::expr_t::numeric_constant_t>(&expr.value);
    if (not c)
...
boost::test_tools::predicate_result is_zero(dep0::parser::expr_t const& expr)
{
    return is_numeric_constant(expr, "0");
}

boost::test_tools::predicate_result is_return_of_var(dep0::parser::stmt_t const& stmt, std::string_view const name)
{
    auto failed = boost::test_tools::predicate_result(false);
    auto const ret = std::get_if<dep0::parser::stmt_t::return_t>(&stmt.value);
    if (not ret)
        failed.message().stream() << "statement is not return but " << pretty_name(stmt.value);
    else if (not ret->expr.has_value())
        failed.message().stream() << "return statement does not contain an expression";
    else
        return is_var(*ret->expr, name);
    return failed;
}

boost::test_tools::predicate_result is_return_of_zero(dep0::parser::stmt_t const& stmt)
{
    auto failed = boost::test_tools::predicate_result(false);
    auto const ret = std::get_if<dep0::parser::stmt_t::return_t>(&stmt.value);
    if (not ret)
...
BOOST_AUTO_TEST_CASE(test_0178)
{
    ...
    }
    {
        ...
        BOOST_TEST(is_var(expr->func.get(), "id"));
        BOOST_TEST_REQUIRE(expr->args.size() == 2ul);
        BOOST_TEST(is_var(expr->args[0ul], "int"));
        BOOST_TEST(is_var(expr->args[1ul], "x"));
    }
    {
    ...
    }
    {
        ...
        BOOST_TEST(is_var(f_int->func.get(), "f"));
        {
            ...
            BOOST_TEST(is_var(f_int->args[0ul], "int"));
        ...
        BOOST_TEST(is_var(f_t->args[0ul], "t"));
    ...
    }
    {
        ...
        BOOST_TEST(f.value.args.size() == 0ul);
        auto const ret_type = std::get_if<dep0::parser::type_t::arr_t>(&f.value.ret_type.value);
        BOOST_TEST_REQUIRE(ret_type);
        BOOST_TEST_REQUIRE(ret_type->arg_types.size() == 2ul);
        auto const ret_arg_0 = std::get_if<dep0::parser::type_t::var_t>(&ret_type->arg_types[0]);
        auto const ret_arg_1 = std::get_if<dep0::parser::type_t>(&ret_type->arg_types[1]);
...
BOOST_AUTO_TEST_CASE(test_0179)
{
    ...
    BOOST_TEST(is_type_var(succ.value.args[1ul].sort, "t"));
    BOOST_TEST_REQUIRE(succ.value.body.stmts.size() == 1ul);
    auto const ret = std::get_if<dep0::parser::stmt_t::return_t>(&succ.value.body.stmts[0ul].value);
    BOOST_TEST_REQUIRE(ret);
    BOOST_TEST_REQUIRE(ret->expr.has_value());
    auto const expr = std::get_if<dep0::parser::expr_t::arith_expr_t>(&ret->expr->value);
    BOOST_TEST_REQUIRE(expr);
    auto const plus = std::get_if<dep0::parser::expr_t::arith_expr_t::plus_t>(&expr->value);
    BOOST_TEST_REQUIRE(plus);
    BOOST_TEST(is_var(plus->lhs.get(), "x"));
    BOOST_TEST(is_numeric_constant(plus->rhs.get(), "1"));
}
...
BOOST_AUTO_TEST_CASE(test_0181)
{
    ...
    }
    {
        ...
        BOOST_TEST_REQUIRE(f.value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of_var(f.value.body.stmts[0ul], "x"));
    }
    {
        ...
        auto const ret = std::get_if<dep0::parser::stmt_t::return_t>(&g.value.body.stmts[0ul].value);
        BOOST_TEST_REQUIRE(ret);
        BOOST_TEST_REQUIRE(ret->expr.has_value());
        auto const call = std::get_if<dep0::parser::expr_t::app_t>(&ret->expr->value);
        BOOST_TEST_REQUIRE(call);
        ...
        BOOST_TEST(is_var(call->args[0], "int"));
...
BOOST_AUTO_TEST_CASE(test_0182)
{
    ...
    }
    {
        ...
        BOOST_TEST(is_typename(f.value.args[0ul].sort));
        {
            auto const type = std::get_if<dep0::parser::type_t>(&f.value.args[1ul].sort);
            BOOST_TEST_REQUIRE(type);
        ...
        BOOST_TEST_REQUIRE(f.value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of_zero(f.value.body.stmts[0ul]));
    }
    {
        ...
        auto const call = std::get_if<dep0::parser::expr_t::app_t>(&ret->expr->value);
        BOOST_TEST_REQUIRE(call);
        BOOST_TEST(is_var(call->func.get(), "g"));
        BOOST_TEST_REQUIRE(call->args.size() == 2ul);
        BOOST_TEST(is_var(call->args[0], "int"));
        BOOST_TEST(is_var(call->args[1], "id"));
    }
}
