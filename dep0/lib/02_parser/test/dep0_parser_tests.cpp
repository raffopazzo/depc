...
BOOST_AUTO_TEST_CASE(test_0178)
{
    ...
    BOOST_TEST_REQUIRE(pass_result->func_defs.size() == 24ul);
    {
        ...
        auto const s1 = std::get_if<dep0::parser::type_t>(&f.value.args[1ul].sort);
        ...
        BOOST_TEST_REQUIRE(ret);
        BOOST_TEST_REQUIRE(ret->expr.has_value());
        auto const expr = std::get_if<dep0::parser::expr_t::var_t>(&ret->expr->value);
        BOOST_TEST_REQUIRE(expr);
    ...
    }
    {
        ...
        BOOST_TEST(std::holds_alternative<dep0::parser::type_t::i32_t>(arg0->value));
    ...
    }
    {
        ...
        auto const ret_type = std::get_if<dep0::parser::type_t::var_t>(&f.value.ret_type.value);
    ...
    }
    {
        ...
        auto const arg0 = std::get_if<dep0::parser::type_t>(&expr->args[0ul].value);
        auto const arg1 = std::get_if<dep0::parser::expr_t::var_t>(&expr->args[1ul].value);
        auto const arg2 = std::get_if<dep0::parser::expr_t::numeric_constant_t>(&expr->args[2ul].value);
        BOOST_TEST_REQUIRE(arg0);
        BOOST_TEST_REQUIRE(arg1);
