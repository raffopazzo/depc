...
BOOST_AUTO_TEST_CASE(test_0178)
{
    ...
    auto const s1 = std::get_if<dep0::parser::type_t>(&id.args[1ul].sort);
    ...
    auto const ret_id = std::get_if<dep0::parser::stmt_t::return_t>(&id.body.stmts[0ul].value);
    ...
    BOOST_TEST(f.name == "f");
    BOOST_TEST_REQUIRE(f.body.stmts.size() == 1ul);
    auto const ret_f = std::get_if<dep0::parser::stmt_t::return_t>(&f.body.stmts[0ul].value);
