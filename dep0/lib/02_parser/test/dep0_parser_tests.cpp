...
BOOST_AUTO_TEST_CASE(test_0178)
{
    ...
    auto const ret_h = std::get_if<dep0::parser::stmt_t::return_t>(&h.value.body.stmts[0ul].value);
    BOOST_TEST_REQUIRE(ret_h);
    BOOST_TEST_REQUIRE(ret_h->expr.has_value());
    auto const expr_h = std::get_if<dep0::parser::expr_t::app_t>(&ret_h->expr->value);
    BOOST_TEST_REQUIRE(expr_h);
    BOOST_TEST_REQUIRE(expr_h->name == "id");
    BOOST_TEST_REQUIRE(expr_h->args.size() == 2ul);
    auto const expr_h_arg0 = std::get_if<dep0::parser::expr_t::var_t>(&expr_h->args[0ul].value);
    auto const expr_h_arg1 = std::get_if<dep0::parser::expr_t::var_t>(&expr_h->args[1ul].value);
    BOOST_TEST_REQUIRE(expr_h_arg0);
    BOOST_TEST_REQUIRE(expr_h_arg1);
    BOOST_TEST(expr_h_arg0->name == "int");
...
BOOST_AUTO_TEST_CASE(test_0179)
{
    ...
    BOOST_TEST(succ.value.args[0ul].name == "t");
    BOOST_TEST(succ.value.args[1ul].name == "x");
    BOOST_TEST(std::holds_alternative<dep0::ast::typename_t>(succ.value.args[0ul].sort));
    auto const s1 = std::get_if<dep0::parser::type_t>(&succ.value.args[1ul].sort);
    BOOST_TEST_REQUIRE(s1);
    auto const t1 = std::get_if<dep0::parser::type_t::var_t>(&s1->value);
    BOOST_TEST_REQUIRE(t1);
    BOOST_TEST_REQUIRE(t1->name == "t");
    ...
    BOOST_TEST_REQUIRE(expr);
    auto const plus = std::get_if<dep0::parser::expr_t::arith_expr_t::plus_t>(&expr->value);
    auto const x = std::get_if<dep0::parser::expr_t::var_t>(&plus->lhs.get().value);
    BOOST_TEST_REQUIRE(x);
    BOOST_TEST(x->name == "x");
