#define BOOST_TEST_MODULE dep0_parser_tests_0010_axioms
...
BOOST_FIXTURE_TEST_SUITE(dep0_parser_tests_0010_axioms, ParserTestsFixture)
...
{
    BOOST_TEST_REQUIRE(pass("0010_axioms/pass_003.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        auto const f = std::get_if<dep0::parser::axiom_t>(&pass_result->entries[0ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(is_true_t_of(f->signature.ret_type.get(), lt(var("a"), plus(var("a"), constant(1)))));
    }
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[1ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(is_arg(f->value.args[1], array_of(is_i32, plus(var("n"), constant(1))), "xs"));
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], subscript_of(var("xs"), var("n"))));
    }
}

BOOST_AUTO_TEST_CASE(typecheck_error_000)
{
    BOOST_TEST_REQUIRE(pass("0010_axioms/typecheck_error_000.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        auto const f = std::get_if<dep0::parser::axiom_t>(&pass_result->entries[0ul]);
        BOOST_TEST_REQUIRE(f);
...
{
    BOOST_TEST_REQUIRE(pass("0010_axioms/typecheck_error_001.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        auto const f = std::get_if<dep0::parser::axiom_t>(&pass_result->entries[0ul]);
        BOOST_TEST_REQUIRE(f);
...
{
    BOOST_TEST_REQUIRE(pass("0010_axioms/typecheck_error_002.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        auto const f = std::get_if<dep0::parser::axiom_t>(&pass_result->entries[0ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST_REQUIRE(f->signature.args.size() == 1ul);
        BOOST_TEST(is_arg(f->signature.args[0], is_u64, "a"));
        BOOST_TEST(is_true_t_of(f->signature.ret_type.get(), lt(var("a"), var("a"))));
    }
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[1ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], subscript_of(var("xs"), constant(0))));
    }
}
