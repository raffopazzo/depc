...
BOOST_AUTO_TEST_CASE(pass_002)
{
    ...
    }
    {
        ...
        BOOST_TEST(
            is_return_of(
                f->value.body.stmts[0],
                app_of(var("f"), pi_of(std::tuple{arg_of(array_of(is_i32, var("n")))}, is_i32))));
    }
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[2ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], app_of(var("f"), array_of(is_i32, var("n")))));
    }
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[3ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], app_of(var("f"), true_t_of(gt(var("n"), constant(0))))));
    }
}
...
BOOST_AUTO_TEST_CASE(pass_003)
{
    ...
    }
    {
        ...
        BOOST_TEST(is_arg(f->value.args[1], app_of(var("f"), subscript_of(var("xs"), constant(0))), std::nullopt));
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], constant(0)));
    }
...
{
    BOOST_TEST_REQUIRE(pass("0011_quantities/typecheck_error_003.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[0ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], plus(var("x"), constant(1))));
    }
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[1ul]);
        BOOST_TEST_REQUIRE(f);
        ...
        BOOST_TEST(
            is_if_else(
                ...
                std::tuple{
                    if_else_of(
                        gt(app_of(var("f"), var("y")), constant(0)),
                        std::tuple{return_of(constant(1))})}));
        BOOST_TEST(is_return_of(f->value.body.stmts[1], var("y")));
    }
}

BOOST_AUTO_TEST_CASE(typecheck_error_004)
{
    BOOST_TEST_REQUIRE(pass("0011_quantities/typecheck_error_004.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[0ul]);
        BOOST_TEST_REQUIRE(f);
