 ...
 */
#define BOOST_TEST_MODULE dep0_parser_tests_0018_mult_expr
...
BOOST_FIXTURE_TEST_SUITE(dep0_parser_tests_0018_mult_expr, ParserTestsFixture)
...
{
    BOOST_TEST_REQUIRE(pass("0018_mult_expr/typecheck_error_000.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 1ul);
    auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[0]);
    BOOST_TEST_REQUIRE(f);
    BOOST_TEST(f->name == "minus_two");
