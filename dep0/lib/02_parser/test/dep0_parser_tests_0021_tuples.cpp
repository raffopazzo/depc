...
BOOST_AUTO_TEST_CASE(pass_004)
{
    ...
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 5ul);
    {
        ...
        BOOST_TEST(
            is_if_else(
                f->value.body.stmts[0ul],
                lt(constant(2), subscript_of(var("x"), constant(0))),
                std::tuple{return_of(subscript_of(subscript_of(var("x"), constant(1)), constant(2)))},
    ...
    }
    {
        ...
        BOOST_TEST(
            is_arg(
                ...
                "x"));
        BOOST_TEST(is_array_of(f->value.ret_type.get(), is_i32, subscript_of(var("x"), constant(0))));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0ul], subscript_of(var("x"), constant(1))));
    }
    {
    ...
    }
    {
        ...
        BOOST_TEST(
            is_array_of(
                f->value.ret_type.get(),
                sigma_of(std::tuple{arg_of(is_u64, "n"), arg_of(array_of(is_i32, var("n")))}),
                constant(3)));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(
            is_return_of(
