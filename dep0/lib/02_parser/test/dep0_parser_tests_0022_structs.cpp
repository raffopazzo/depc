...
{
    BOOST_TEST_REQUIRE(pass("0022_structs/pass_010.depc"));
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    BOOST_TEST(
        is_struct_def(
            pass_result->entries[0], "t",
            struct_field("f", pi_of(std::tuple{}, is_i32)),
            struct_field("v", array_of(is_i32, constant(1)))));
    {
        auto const f = std::get_if<dep0::parser::func_def_t>(&pass_result->entries[1]);
        BOOST_TEST_REQUIRE(f);
        BOOST_TEST(f->name == "f");
        BOOST_TEST_REQUIRE(f->value.args.size() == 1ul);
        BOOST_TEST(is_arg(f->value.args[0], array_of(var("t"), constant(1)), "x"));
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        auto const x0 = subscript_of(var("x"), constant(0));
        BOOST_TEST(
            is_return_of(
                f->value.body.stmts[0ul],
                plus(
                    app_of(member_of(x0, "f")),
                    subscript_of(member_of(x0, "v"), constant(0)))));
    }
}
