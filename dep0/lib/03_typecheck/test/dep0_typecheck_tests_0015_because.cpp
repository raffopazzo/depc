 ...
 */
#define BOOST_TEST_MODULE dep0_typecheck_tests_0015_because
...
BOOST_FIXTURE_TEST_SUITE(dep0_typecheck_tests_0015_because, TypecheckTestsFixture)
            ...
            is_return_of(
                ...
                because_of(
                    ...
                    app_of(
                        ...
                        var("n"),
                        init_list_of(),
                        expr_of_type(is_unnamed_var, true_t_of(lt(constant(2), var("n"))))))));
    }
    {
            ...
            is_return_of(
                ...
                because_of(
                    ...
                    app_of(
                        ...
                        constant(2),
                        var("n"),
                        expr_of_type(is_unnamed_var, true_t_of(lt(var("i"), constant(2)))),
                        expr_of_type(is_unnamed_var, true_t_of(lt(constant(2), var("n"))))))));
    }
    {
    ...
    }
    {
        ...
        BOOST_TEST(
            is_return_of(
                ...
                because_of(
                    constant(0),
                    because_of(constant(true), app_of(global("rand"))))));
    }
}
...
BOOST_AUTO_TEST_CASE(pass_002)
{
    ...
    }
    {
        ...
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0], app_of(global("f1"))));
    }
}
...
BOOST_AUTO_TEST_CASE(pass_003)
{
    ...
    BOOST_TEST_REQUIRE(pass_result->entries.size() == 2ul);
    {
        ...
        BOOST_TEST_REQUIRE(f->value.args.size() == 3ul);
        BOOST_TEST(is_arg(f->value.args[0], is_u64, "n"));
        BOOST_TEST(is_arg(f->value.args[1], because_of(array_of(is_i32, var("n")), constant(true)), "xs"));
        BOOST_TEST(is_arg(f->value.args[2], true_t_of(lt(constant(0), var("n"))), std::nullopt, dep0::ast::qty_t::zero));
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
    ...
    }
    {
        ...
        BOOST_TEST(
            is_arg(
                f->value.args[1],
                array_of(
                    because_of(array_of(is_i32, var("n")), constant(true)),
                    var("n")),
                "xs"));
        BOOST_TEST(is_arg(f->value.args[2], true_t_of(lt(constant(1), var("n"))), std::nullopt, dep0::ast::qty_t::zero));
        BOOST_TEST(is_i32(f->value.ret_type.get()));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(
            is_return_of(
                f->value.body.stmts[0],
                subscript_of(subscript_of(var("xs"), constant(1)), constant(1))));
    }
}
