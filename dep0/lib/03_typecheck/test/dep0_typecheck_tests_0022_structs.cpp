 ...
 */
#define BOOST_TEST_MODULE dep0_typecheck_tests_0022_structs
...
BOOST_FIXTURE_TEST_SUITE(dep0_typecheck_tests_0022_structs, TypecheckTestsFixture)
...
BOOST_AUTO_TEST_CASE(pass_000)
{
    ...
    BOOST_TEST(is_struct_def(pass_result->entries[2], "v", struct_field("x", is_i32), struct_field("u", global("u"))));
    {
    ...
    }
    {
        ...
        BOOST_TEST(f->value.args.size() == 0ul);
        BOOST_TEST(is_global(f->value.ret_type.get(), "u"));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
    ...
    }
    {
        ...
        BOOST_TEST(f->value.args.size() == 0ul);
        BOOST_TEST(is_global(f->value.ret_type.get(), "v"));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(
            is_return_of(
    ...
    }
    {
        ...
        BOOST_TEST(is_global(f->value.ret_type.get(), "v"));
        BOOST_TEST_REQUIRE(f->value.body.stmts.size() == 1ul);
        BOOST_TEST(is_return_of(f->value.body.stmts[0ul], init_list_of(constant(2), app_of(global("f1")))));
    }
}
...
BOOST_AUTO_TEST_CASE(pass_001)
{
            ...
            struct_field("v", array_of(is_i32, var("n")))));
    {
        ...
        BOOST_TEST(is_arg(f->value.args[0], global("t"), "x"));
...
BOOST_AUTO_TEST_CASE(typecheck_error_002) { BOOST_TEST(fail("0022_structs/typecheck_error_002.depc")); }
BOOST_AUTO_TEST_CASE(typecheck_error_003) { BOOST_TEST(fail("0022_structs/typecheck_error_003.depc")); }
BOOST_AUTO_TEST_CASE(typecheck_error_004) { BOOST_TEST(fail("0022_structs/typecheck_error_004.depc")); }
