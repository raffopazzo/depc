...
bool is_first_order_type(typecheck::expr_t const& type)
{
    return match(
        ...
        {
            // TODO only if no dependency? no, because the existential quantifier is part of first-order logic
            // higher-order logic takes a predicate, eg `(type p, p x)` reads "exists proposition p with a proof x"
