...
boost::test_tools::predicate_result is_and_of(llvm::Instruction const& x, F1&& f1, F2&& f2)
{
    ...
    if (op->getOpcode() != llvm::Instruction::BinaryOps::And)
        return dep0::testing::failure(
            "binary operator is not And: ",
            static_cast<int>(llvm::Instruction::BinaryOps::And),
            " != ",
            static_cast<int>(op->getOpcode()));
    if (op->getNumOperands() != 2)
        return dep0::testing::failure("`and` does not have 2 operands but ", op->getNumOperands());
...
template <Predicate<llvm::Value> F1, Predicate<llvm::Value> F2>
boost::test_tools::predicate_result is_and_of(llvm::Instruction const* const p, F1&& f1, F2&& f2)
{
    if (not p)
        return dep0::testing::failure("instruction is null");
    return is_and_of(*p, std::forward<F1>(f1), std::forward<F2>(f2));
}

template <Predicate<llvm::Value> F1, Predicate<llvm::Value> F2>
boost::test_tools::predicate_result is_and_of(llvm::Value const& x, F1&& f1, F2&& f2)
{
...
template <Predicate<llvm::Value> F1, Predicate<llvm::Value> F2>
boost::test_tools::predicate_result is_and_of(llvm::Value const* const p, F1&& f1, F2&& f2)
{
    if (not p)
...
inline constexpr auto is_and_of = boost::hana::overload(
    [] <Predicate<llvm::Value> F1, Predicate<llvm::Value> F2>
    (llvm::Instruction const& x, F1&& f1, F2&& f2)
    {
        return impl::is_and_of(x, std::forward<F1>(f1), std::forward<F2>(f2));
    },
    [] <Predicate<llvm::Value> F1, Predicate<llvm::Value> F2>
    (llvm::Instruction const* const p, F1&& f1, F2&& f2)
    {
        return impl::is_and_of(p, std::forward<F1>(f1), std::forward<F2>(f2));
    },
    [] <Predicate<llvm::Value> F1, Predicate<llvm::Value> F2>
