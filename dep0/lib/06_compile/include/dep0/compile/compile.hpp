 ...
 * return a temporaty file object associated with it.
 *
 * @remarks It is the caller responsibility to keep the temporary file if required.
 */
expected<temp_file_t> compile_only(llvm::Module&, llvm::TargetMachine&);
 ...
 * @remarks It is the caller responsibility to keep the temporary file if required.
 */
expected<temp_file_t> compile_and_assemble(llvm::Module&, llvm::TargetMachine&);
